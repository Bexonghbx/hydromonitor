...
class DB:
    ...
    def humidityMMAR(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.climo.aggregate([{'$match':{'timestamp':{'$gte':int(start),'$lte':int(end)}}},{'$group':{'_id': "humidity",'humidity':{'$push':"$$ROOT.humidity"}}},{'$project':{'max':{'$max':'$humidity'},'min':{'$min':'$humidity'},'avg':{'$avg':'$humidity'},'range':{'$subtract':[{'$max':'$humidity'},{'$min':'$humidity'}]}}}]))
        except Exception as e:
            msg = str(e)
    ...
    def temperatureMMAR(self,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.climo.aggregate( [{'$match':{'timestamp':{'$gte':int(start),'$lte':int(end)}}},{'$group':{'_id': "temperature",'temperature':{'$push':"$$ROOT.temperature"}}},{'$project':{'max':{'$max':'$temperature'},'min':{'$min':'$temperature'},'avg':{'$avg':'$temperature'},'range':{'$subtract':[{'$max':'$temperature'},{'$min':'$temperature'}]}}}]))
        except Exception as e:
            msg = str(e)
    ...
    def frequencyDistro(self,variable,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.climo.aggregate( [{'$match':{'timestamp':{'$gte':int(start),'$lte':int(end)}}},{'$bucket':{'groupBy': "$"+variable, 'boundaries': list(range(0,101)), 'default': "outliers", 'output': {"count": { '$sum': 1 }}}}]))
        except Exception as e:
            msg = str(e)
